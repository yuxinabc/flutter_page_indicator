...
class ScalePainter extends BasePainter {
  ...
  void draw(Canvas canvas, double space, double size, double radius) {
    ...
    double progress = page - index;
    _paint.color = Color.lerp(widget.activeColor, widget.color, progress)??Color.fromARGB(1, 0, 0, 0);
    //last
    canvas.drawCircle(new Offset(radius + (index * (size + space)), radius),
        lerp(radius, radius * widget.scale, progress), _paint);
    //first
    _paint.color = Color.lerp(widget.color, widget.activeColor, progress)??Color.fromARGB(1, 0, 0, 0);
